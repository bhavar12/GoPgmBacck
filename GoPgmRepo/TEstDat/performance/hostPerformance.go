...
const (
	...
	vmProp              = "vm"
	apiProtocol         = "https://"
	usageNoneConst      = "cpu.usage.none"
	usageAverageConst   = "cpu.usage.average"
	usageSummationConst = "cpu.ready.summation"
)
...
func VirtualMachinePerformance(ctx context.Context, c *govmomi.Client, hs []mo.HostSystem) {
	var (
		...
		spec               = types.PerfQuerySpec{Format: string(types.PerfFormatNormal)}
	)
...
func HostPerformance(ctx context.Context, c *govmomi.Client, hs mo.HostSystem) {
	...
	// }
	entities = append(entities, hs.Reference())
	...
	for _, metric := range result {
		...
		for _, v := range metric.Value {
			...
			if len(v.Value) != 0 {
				// if v.Name == usageNoneConst {
				// 	metricsValue := v.Value[len(v.Value)-1]
				// 	fmt.Printf("%s\t%s\t%s\t%d\n",
				// 		name, instance, v.Name, metricsValue)
				// } else {
				// 	metricsValue := v.Value[len(v.Value)-1]
				// 	fmt.Printf("%s\t%s\t%s\t%d\n",
				// 		name, instance, v.Name, metricsValue)
...
func main() {
	...
	for _, val := range hostSys {
		fmt.Println("Data for the host", val.Hardware.SystemInfo.Uuid)
		//HostPerformance(ctx, conn, val)
		PerfRun(ctx, conn.Client, val)
	}
...
func PerfRun(ctx context.Context, c *vim25.Client, hs mo.HostSystem) error {
	var (
		metrics  = make([]string, 0)
		entities = make([]types.ManagedObjectReference, 0)
	)
